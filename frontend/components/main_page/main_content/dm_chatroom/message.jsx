...
class Message extends React.Component {
  ...
  handleClick(e) {
    ...
    let dmGroup_id = this.props.dmGroup.id;
    let sub;
    App.cable.subscriptions.subscriptions.forEach((subs, idx) => {
      
      sub = JSON.parse(subs.identifier);
      if (sub.id === dmGroup_id && sub.channel === 'DmChatChannel') {

        App.cable.subscriptions.subscriptions[idx].updateDM({
          message: message
        })
      }
    })
  ...
  handleDelete(e) {
    ...
    App.cable.subscriptions.subscriptions.forEach((subs, idx) => {
      ...
      if (sub.id === dmGroup_id && sub.channel === 'DmChatChannel') {
        ...
        App.cable.subscriptions.subscriptions[idx].deleteDM({
          message: message
        })
      }
    })
